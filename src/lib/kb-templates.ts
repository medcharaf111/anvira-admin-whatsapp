...
const REAL_ESTATE_KSA: KbTemplate = {
  ...
  fields: {
    ...
    about:
      'مكتب وساطة عقارية في المملكة العربية السعودية [أدخل حالة الترخيص لدى الهيئة العامة للعقار — REGA]. نخدم مشتري Vision 2030 من سكان الرياض وجدة والدمام، إضافة للمستثمرين الخليجيين والمصريين والمقيمين. تخصصنا في المشاريع الكبرى: ROSHN، الدار العقارية، Retal Urban Development.',
    property_types_handled:
...
- فيلا دوبلكس: SAR 120K – 180K`,
    commission_structure: `العمولة (يرجى التحقق من المتطلبات الحالية لدى الهيئة العامة للعقار — REGA):
...
لا نأخذ أي رسوم تسجيل أو "حجز" قبل المعاينة وتوقيع عقد الوساطة [راجع نموذج العقد المعتمد لدى REGA].`,
...
- شركات (مكاتب إدارية في KAFD / ITCC)`,
    broker_license: `[أدخل رقم رخصة الفال إن وُجد] REGA FAL License:
...
[راجع متطلبات الإعلان الحالية لدى الهيئة العامة للعقار — REGA قبل نشر أي إعلان].`,
...
كل وسطائنا مسجّلين في الهيئة العامة للعقار برقم فال فردي.`,
    policies: `Cooling-off period:
- Off-plan: [راجع فترة حق الانسحاب الحالية المقرّرة من الهيئة العامة للعقار — REGA].
...
Escrow & دفعات:
- مدفوعات Off-plan: [راجع متطلبات حساب الضمان (Escrow) الحالية لدى الجهات المختصة قبل أي تحويل].
...
- نلتزم بمتطلبات حماية البيانات السعودية (PDPL). تفضيل منطقة تخزين البيانات يُحدَّد مع فريق Anvira وليس ضماناً تعاقدياً بموقع التخزين.

عقد الوساطة:
- [راجع نموذج عقد الوساطة المعتمد الحالي] يوقّعه البائع والمشتري قبل بدء أي تسويق
...
ج: العقود الرسمية باللغة العربية. نوفر ترجمة إنجليزية تفسيرية، والنسخة العربية هي المعتمدة قانونياً. [راجع المتطلبات الحالية لدى الجهات المختصة].`,
  },
};
 ...
 */
// Item 15 — Saudi KB templates are QUARANTINED (not deleted) behind a beta
// flag. REAL_ESTATE_KSA is excluded from the picker unless an operator has
// explicitly opted into the KSA beta, because templatesFor() does NOT filter
// by country and would otherwise surface the KSA template to every UAE
// real_estate tenant. Set NEXT_PUBLIC_KSA_KB_BETA=1 to re-enable it (e.g. in
// a controlled KSA pilot once the claims are verified).
const KSA_KB_BETA = process.env.NEXT_PUBLIC_KSA_KB_BETA === '1';
const QUARANTINED_TEMPLATE_IDS = new Set(['real_estate_ksa']);

export function templatesFor(clientType: ClientType): KbTemplate[] {
  return KB_TEMPLATES.filter(
    (t) =>
      t.client_type === clientType &&
      (KSA_KB_BETA || !QUARANTINED_TEMPLATE_IDS.has(t.id))
  );
}
