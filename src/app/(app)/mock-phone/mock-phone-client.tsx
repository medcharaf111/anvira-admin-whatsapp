...
export interface Persona {
  name: string;
  phone: string;
  /** Optional opener — shown as a one-click insert button. */
  opener?: string;
}
...
export function MockPhoneClient({
  personas,
  clientType,
}: {
  personas: Persona[];
  ...
  const subtitle =
    clientType === 'real_estate'
      ? 'جرّب مساعدك من منظور المشتري العقاري. اختر شخصية وابدأ الدردشة.'
  ...
  return (
    <div>
      ...
      {persona.opener && (
        <div
          className="max-w-md mx-auto mb-3 p-3 text-xs flex items-start gap-2"
          style={{
        ...
        >
          <div className="flex-1">
            ...
            <div>{persona.opener}</div>
          </div>
          <button
            onClick={() => send(persona.opener)}
